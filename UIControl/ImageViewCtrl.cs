...
namespace sssongVision.UIControl
{
    ...
    public partial class ImageViewCtrl : UserControl
    {
        ...
        private const float MaxZoom = 100.0f;  //최대 줌 제한 값
        ...
        private void DrawDiagram(Graphics g)
        {
            ...
            foreach (DiagramEntity entity in _diagramEntityList)
            {
                ...
                {
                    if (_multiSelectedEntities.Contains(entity)) // 영역 2개 이상 선택 됐을때
                    {
                        pen.DashStyle = DashStyle.Dash; // 포함되면 점선으로 그려라~ 하는 의미임 dashStyle
                        pen.Width = 2;
        ...
        // 이진화 검사 : 화면에 보여줄 영역 정보를 표시하기 위해, 위치 입력 받는 함수
