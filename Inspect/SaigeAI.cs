...
namespace sssongVision.Inspect
{
    ...
    public class SaigeAI : IDisposable
    {
        ...
        // Model Load (engine)
        public void LoadEngine(string modelPath, ModelType modelType)
        {
            ...
            switch (_modelType)
            {
                ...
                case ModelType.SEG:
                    ...
                    break;
                default:
                    throw new NotSupportedException("지원하지 않는 모델 타입입니다.");
            }
        }

        public void LoadIADEngine(string modelPath)
        {
            // 검사하기 위한 엔진에 대한 객체 생성
            // 인스턴스 생성 시 모델 파일 정보와 GPU Index 입력, 필요에 따라 batch size 입력
            _iADEngine = new IADEngine(modelPath, 0);

            IADOption option = _iADEngine.GetInferenceOption();
            option.CalcScoremap = false;

            // 약간의 속도차이로 불필요할경우 false 해두면 됨. 필요하면 true 하고~
            option.CalcHeatmap = false; 
            option.CalcMask = false;
            option.CalcObject = true;

            // Segmented object의 면적이 object area/score threshold 보다 작으면 최종 결과에서 제외
        ...
        public bool InspAIModule(Bitmap bmpImage)
        {
            ...
            switch (_modelType)
            {
                case ModelType.IAD:
                    ...
                    if (_iADEngine == null)
                    {
                        MessageBox.Show("엔진이 초기화되지 않았습니다. LoadEngine 메서드를 호출하여 엔진을 초기화하세요.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
