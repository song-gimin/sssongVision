...
{
    // #7_BINARY_PREVIEW# 이진화 Preview 구현
    ...
    // Core - Define.cs 생성 : 프로그램 전체적으로 전역 설정이나, 타입을 정의하기 위한 클래스 
    // Algorithm - DrawInspectInfo.cs 생성 : 검사 결과 영역을 그리기 위한 클래스
    // BinaryProp 디자인창을 통해서, 검사 속성 추가
    ...
    public class BlobAlgorithm : InspAlgorithm
    {
        ...
        private bool InspPixelCount (Mat binImage)
        {
            ...
            // 흰색 픽셀(255) 총 갯수 계산
            int pixelCnt = Cv2.CountNonZero(binImage);

            _findArea.Clear();

            IsDefect = false;
            string result = "OK";
            string featureInfo = $"A:{pixelCnt}";

            BlobFilter areaFilter = BlobFilters[FILTER_AREA];

            if (areaFilter.isUse)
            {
                if ((areaFilter.min > 0 && pixelCnt < areaFilter.min) ||
                    (areaFilter.max > 0 && pixelCnt > areaFilter.max))
                {
                    IsDefect = true;
                    result = "NG";
                }
            ...
            {
                string resultInfo = "";
                resultInfo = $"[{result}] Blob count [in : {areaFilter.min},{areaFilter.max},out : {pixelCnt}]";
        ...
        // 이진화후, Blob을 찾아서, 그 특징값이 필터된 것을 찾음
        private bool InspBlobFilter (Mat binImage)
        {
            ...
            if (_findArea is null) _findArea = new List<DrawInspectInfo>();
